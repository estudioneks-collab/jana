...
import { ShoppingBag, Search, X, MessageCircle, Leaf, Star, Heart, ArrowRight, ChevronLeft, ChevronRight } from 'lucide-react';
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-5xl bg-[#F2EFED] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row animate-in zoom-in-95 duration-500 max-h-[90vh]">
            <button 
              ...
              className="absolute top-4 right-4 z-20 w-10 h-10 bg-white/90 rounded-2xl flex items-center justify-center text-[#2C3E50] shadow-lg hover:scale-110 transition-all"
            >
              <X size={20} />
            </button>
              ...
              {selectedProduct.imageUrls && selectedProduct.imageUrls.length > 0 ? (
                <>
                  ...
                  {selectedProduct.imageUrls.length > 1 && (
                    <>
                      <button onClick={prevImg} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/50 backdrop-blur rounded-full flex items-center justify-center text-[#2C3E50] hover:bg-white transition-all shadow-md">
                        <ChevronLeft size={20} />
                      </button>
                      <button onClick={nextImg} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/50 backdrop-blur rounded-full flex items-center justify-center text-[#2C3E50] hover:bg-white transition-all shadow-md">
                        <ChevronRight size={20} />
                      </button>
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                         {selectedProduct.imageUrls.map((_, i) => (
                           <div key={i} className={`w-2 h-2 rounded-full transition-all ${i === currentImgIdx ? 'bg-[#5D7F8E] w-4' : 'bg-slate-200'}`} />
                         ))}
                      </div>
                    </>
                  )}
            ...
            {/* Columna Info */}
            <div className="w-full md:w-1/2 p-8 md:p-16 flex flex-col overflow-y-auto custom-scrollbar bg-white md:bg-transparent">
              <div className="mb-10">
                <span className="bg-[#5D7F8E] text-white px-3 py-1 rounded-full text-[8px] font-bold uppercase tracking-[0.2em] mb-4 inline-block shadow-sm">
                ...
                <div className="bg-[#F2EFED]/50 p-6 rounded-3xl inline-block border border-white">
              ...
              {cart.length === 0 ? (
                ...
                cart.map(item => (
                  <div key={item.product.id} className="flex gap-4 group">
                    <div className="w-20 h-20 bg-slate-50 rounded-2xl overflow-hidden shrink-0 shadow-inner flex items-center justify-center p-1">
                      {(item.product.imageUrls?.length > 0 || item.product.imageUrl) && (
                        <img src={item.product.imageUrls?.[0] || item.product.imageUrl} className="w-full h-full object-contain" />
                      )}
                    </div>
